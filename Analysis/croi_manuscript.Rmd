...
sa.differences.table %>% write_csv("sensitivity_analysis_table_20240827.csv")

```

### Spot checking metrics

For some reason, seeing misaligned sensitivity analysis for cmdinc and for delays to art uptake
...
```{r}
sa.differences.table %>% filter(Years == "1985-2035", Ages == "15-100") %>% head(11)
```
...
center.infs = sa.differences.table %>% filter(Period == "Universal ART", Ages == "15-100", scenario_name == "baseline.art") %>% .$Newly.Infected.pct

p.sa.infections <- ggplot() + 
  geom_rect(data = sa.differences.table %>% filter(Period == "Universal ART", Ages == "15-100", limitsign == 0),
            mapping = aes(ymin = -ix, ymax =- ix - 1, 
                          xmin = Newly.Infected.pct, 
                          xmax = center.infs), fill = "#00468B") + 
    geom_rect(data = sa.differences.table %>% filter(Period == "Universal ART", Ages == "15-100", limitsign == 1),
            mapping = aes(ymin =- ix, ymax =- ix - 1, 
                          xmin = center.infs, 
...
```{r}
center.deaths = sa.differences.table %>% filter(Period == "Universal ART", Ages == "15-100", scenario_name == "baseline.art") %>% .$Died_from_HIV.pct
...
```{r}
center.dep = sa.differences.table %>% filter(Period == "Universal ART", Ages == "15-100", scenario_name == "baseline.art") %>% .$Depression.eps.pct
...
ggsave("sensitivity_analysis_20240820.pdf", p.sa, width = 9, height = 3, units = 'in')
```
