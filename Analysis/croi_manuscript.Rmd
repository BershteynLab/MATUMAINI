...
sim.deaths.sum <- sim.deaths %>% 
  group_by(Gender, sim.ix, scenario_name) %>% 
  summarize(Died_from_HIV = sum(Died_from_HIV), .groups = 'keep') %>% 
  ungroup()

sim.deaths.mean <- sim.deaths.sum %>% 
  group_by(Gender, scenario_name) %>% 
  summarize(
    Died_from_HIV.sd = sd(Died_from_HIV),
    Died_from_HIV = mean(Died_from_HIV), .groups = "keep") %>% 
  ungroup() %>% 
  mutate(
