...
# experiment_path = "/gpfs/scratch/citrod01/experiments/MATUMAINI/Baseline-campaign_MATUMAINI_202305_test_helloworld-baseline_art_cmdinc_test_helloworld___2024_04_24_14_53_35_336366"
# putting back in art adherence
experiment_path = "/gpfs/scratch/citrod01/experiments/MATUMAINI/Baseline-campaign_MATUMAINI_202305_test_helloworld-baseline_art_cmdinc_test_helloworld___2024_04_24_21_15_08_905770"

sim.results.cmdinc.art.nodropout.test <- EMODAnalyzeR::read.simulation.results.bigpurple(
  experiment_path = experiment_path,
  scenario_name = 'cmdinc.art.nodropout.test',
  summarize_columns = c("Population","Infected", "On_ART",
                        "Died", "Died_from_HIV",
                        "Newly.Infected","Diagnosed"),
  #stratify_columns = c("Year", "Gender", "Age", "IP_Key.CMDStatus"),
...
sim.results.pop.scaling <- sim.results.cmdinc.art.nodropout.test %>% 
      filter(Year == CENSUS_YEAR) %>%
      group_by(sim.id) %>%
      summarize(total.pop = sum(Population), .groups = 'keep') %>% 
      mutate(pop.scaling.factor = KEN_CENSUS_POP/total.pop)

sim.results.cmdinc.art.nodropout.test <- sim.results.cmdinc.art.nodropout.test %>% 
  inner_join(
    sim.results.pop.scaling,
    by = c("sim.id")
  )

# putting back in art dropout
#experiment_path = "/gpfs/scratch/citrod01/experiments/MATUMAINI/Baseline-campaign_MATUMAINI_202305_test_helloworld-baseline_art_cmdinc_test_helloworld___2024_04_25_09_50_58_418277"

# At scale again
#experiment_path = "/gpfs/scratch/citrod01/experiments/MATUMAINI/Baseline-campaign_MATUMAINI_202305_test_helloworld-baseline_art_cmdinc_test_helloworld___2024_04_25_11_27_26_995660"

# putting back in art dropout, but not recalculate
experiment_path = "/gpfs/scratch/citrod01/experiments/MATUMAINI/Baseline-campaign_MATUMAINI_202305_test_helloworld-baseline_art_cmdinc_test_helloworld___2024_04_25_17_59_27_430769"

sim.results.cmdinc.art.dropout.test <- EMODAnalyzeR::read.simulation.results.bigpurple(
  experiment_path = experiment_path,
  scenario_name = 'cmdinc.art.dropout.test',
  summarize_columns = c("Population","Infected", "On_ART",
                        "Died", "Died_from_HIV",
                        "Newly.Infected","Diagnosed"),
  #stratify_columns = c("Year", "Gender", "Age", "IP_Key.CMDStatus"),
...
sim.results.pop.scaling <- sim.results.cmdinc.art.dropout.test %>% 
      filter(Year == CENSUS_YEAR) %>%
      group_by(sim.id) %>%
      summarize(total.pop = sum(Population), .groups = 'keep') %>% 
      mutate(pop.scaling.factor = KEN_CENSUS_POP/total.pop)

sim.results.cmdinc.art.dropout.test <- sim.results.cmdinc.art.dropout.test %>% 
  inner_join(
    sim.results.pop.scaling,
    by = c("sim.id")
  )
...
# Hello World! 1990 up through 2020
...
# putting back in delay to diagnosis
# experiment_path = "/gpfs/scratch/citrod01/experiments/MATUMAINI/Baseline-campaign_MATUMAINI_202305_decouple_test_helloworld-decouple_art_test_helloworld___2024_04_24_14_55_56_088033"
# putting back in art adherence
experiment_path = "/gpfs/scratch/citrod01/experiments/MATUMAINI/Baseline-campaign_MATUMAINI_202305_decouple_test_helloworld-decouple_art_test_helloworld___2024_04_24_21_17_26_424609"

sim.results.decouple.art.nodropout.test <- EMODAnalyzeR::read.simulation.results.bigpurple(
  experiment_path = experiment_path,
  scenario_name = 'decouple.art.nodropout.test',
  summarize_columns = c("Population","Infected", "On_ART",
                        "Died", "Died_from_HIV",
                        "Newly.Infected","Diagnosed"),
  #stratify_columns = c("Year", "Gender", "Age", "IP_Key.CMDStatus"),
...
sim.results.pop.scaling <- sim.results.decouple.art.nodropout.test %>% 
      filter(Year == CENSUS_YEAR) %>%
      group_by(sim.id) %>%
      summarize(total.pop = sum(Population), .groups = 'keep') %>% 
      mutate(pop.scaling.factor = KEN_CENSUS_POP/total.pop)

sim.results.decouple.art.nodropout.test <- sim.results.decouple.art.nodropout.test %>% 
  inner_join(
    sim.results.pop.scaling,
    by = c("sim.id")
  )
...
# putting back in art dropout
#experiment_path = "/gpfs/scratch/citrod01/experiments/MATUMAINI/Baseline-campaign_MATUMAINI_202305_decouple_test_helloworld-decouple_art_test_helloworld___2024_04_25_09_53_53_533674"

# At scale again
#experiment_path = "/gpfs/scratch/citrod01/experiments/MATUMAINI/Baseline-campaign_MATUMAINI_202305_decouple_test_helloworld-decouple_art_test_helloworld___2024_04_25_11_29_53_962972"


# ART dropout only, without recalculation
experiment_path = "/gpfs/scratch/citrod01/experiments/MATUMAINI/Baseline-campaign_MATUMAINI_202305_decouple_test_helloworld-decouple_art_test_helloworld___2024_04_25_18_01_52_065003"

sim.results.decouple.art.dropout.test <- EMODAnalyzeR::read.simulation.results.bigpurple(
  experiment_path = experiment_path,
  scenario_name = 'decouple.art.dropout.test',
  summarize_columns = c("Population","Infected", "On_ART",
                        "Died", "Died_from_HIV",
                        "Newly.Infected","Diagnosed"),
  #stratify_columns = c("Year", "Gender", "Age", "IP_Key.CMDStatus"),
...
sim.results.pop.scaling <- sim.results.decouple.art.dropout.test %>% 
      filter(Year == CENSUS_YEAR) %>%
      group_by(sim.id) %>%
      summarize(total.pop = sum(Population), .groups = 'keep') %>% 
      mutate(pop.scaling.factor = KEN_CENSUS_POP/total.pop)

sim.results.decouple.art.dropout.test <- sim.results.decouple.art.dropout.test %>% 
  inner_join(
    sim.results.pop.scaling,
    by = c("sim.id")
  )
...
sim.results.all.agg.differences = sim.results.all.agg %>% 
  filter(scenario_name == "decouple.art.test") %>% 
