...
sim.results.pop.scaling <- sim.results.baseline.art.cmdinc.test %>% 
      filter(Year == CENSUS_YEAR) %>%
      group_by(sim.id) %>%
      summarize(total.pop = sum(Population), .groups = 'keep') %>% 
      mutate(pop.scaling.factor = KEN_CENSUS_POP/total.pop)

sim.results.baseline.art.cmdinc.test <- sim.results.baseline.art.cmdinc.test %>% 
  inner_join(
    sim.results.pop.scaling,
    by = c("sim.id")
  )
...
sim.results.pop.scaling <- sim.results.decouple.art.test %>% 
      filter(Year == CENSUS_YEAR) %>%
      group_by(sim.id) %>%
      summarize(total.pop = sum(Population), .groups = 'keep') %>% 
      mutate(pop.scaling.factor = KEN_CENSUS_POP/total.pop)

sim.results.decouple.art.test <- sim.results.decouple.art.test %>% 
  inner_join(
    sim.results.pop.scaling,
    by = c("sim.id")
  )
