...
start_year_list = c(1985, 1985, 2000, 2015)
end_year_list = c(2025, 2000, 2015, 2025)
label_list = c("Entire Past Pandemic", "Pre-ART", "ART Scaleup", "Universal ART")
...
### Future pandemic 2025-2050

### Entire Pandemic 1985-2050
...
differences.table %>% arrange(Period, Ages, scenario_name)
...
measure.art.impact <- function(data, colname, start_year, end_year, age_min, age_max){
  ...
  mutate(art.impact = baseline.art.impact - decouple.art.impact,
         art.ppyoa = baseline.art.ppyoa - decouple.art.ppyoa)

  art.impact.deaths.data.agg <- art.impact.deaths.data %>% 
      summarize(baseline.art.impact.sd = sd(baseline.art.impact),
                ...
                art.impact.ppyoa.sd = sd(art.ppyoa),
                art.impact.ppyoa = mean(art.ppyoa), 
                .groups = "keep"
                ) %>%
      ungroup()
  
  bt_resamples <- bootstraps(art.impact.deaths.data, times = 500)
  ...
  bt_resamples$art.impact.ppyoa <- map_dbl(bt_resamples$splits, bt_mean, "art.ppyoa")
  ...
  art.impact.deaths.data.agg$baseline.art.impact_95LL <- baseline.art.impact_95[[1]]
  art.impact.deaths.data.agg$baseline.art.impact_95UL <- baseline.art.impact_95[[2]]
  art.impact.deaths.data.agg$decouple.art.impact_95LL <- decouple.art.impact_95[[1]]
  art.impact.deaths.data.agg$decouple.art.impact_95UL <- decouple.art.impact_95[[2]]
  art.impact.deaths.data.agg$art.impact_95LL <- art.impact_95[[1]]
  art.impact.deaths.data.agg$art.impact_95UL <- art.impact_95[[2]]
  art.impact.deaths.data.agg$baseline.art.ppyoa_95LL <- baseline.art.ppyoa_95[[1]]
  art.impact.deaths.data.agg$baseline.art.ppyoa_95UL <- baseline.art.ppyoa_95[[2]]
  art.impact.deaths.data.agg$decouple.art.ppyoa_95LL <- decouple.art.ppyoa_95[[1]]
  art.impact.deaths.data.agg$decouple.art.ppyoa_95UL <- decouple.art.ppyoa_95[[2]]
  art.impact.deaths.data.agg$art.impact.ppyoa_95LL <- art.impact.ppyoa_95[[1]]
  art.impact.deaths.data.agg$art.impact.ppyoa_95UL <- art.impact.ppyoa_95[[2]]
  
  art.impact.deaths.data.agg <- art.impact.deaths.data.agg %>% select(
...
infections.preart <- rbind(
  measure.art.impact(sim.results.impact, metric, start_year, end_year, 0, 100) %>% 
  mutate(Ages = paste0(0, "-", 100),
         Years = paste0(start_year,"-",end_year)),
  measure.art.impact(sim.results.impact, metric, start_year, end_year, 15, 25) %>% 
  mutate(Ages = paste0(15, "-", 25),
         Years = paste0(start_year,"-",end_year)),
  measure.art.impact(sim.results.impact, metric, start_year, end_year, 25, 100)%>% 
  mutate(Ages = paste0(25, "-", 100),
         Years = paste0(start_year,"-",end_year))
) %>% 
...
### Everything together
```{r}
art.impact.table <- rbind(infections.preart,
      deaths.preart,
      infections.scaleup,
      deaths.scaleup,
      infections.uni,
  ...
  select(
    "Years", "Ages", "Metric",
    "art.impact.ppyoa","art.impact.ppyoa_95LL", "art.impact.ppyoa_95UL","art.impact.ppyoa.sd",
    "art.impact","art.impact_95LL", "art.impact_95UL","art.impact.sd",
    "baseline.art.ppyoa", "baseline.art.ppyoa_95LL", "baseline.art.ppyoa_95UL", "baseline.art.ppyoa.sd", 
    "baseline.art.impact", "baseline.art.impact_95LL", "baseline.art.impact_95UL", "baseline.art.impact.sd", 
    "decouple.art.ppyoa", "decouple.art.ppyoa_95LL", "decouple.art.ppyoa_95UL", "decouple.art.ppyoa.sd", 
    "decouple.art.impact", "decouple.art.impact_95LL", "decouple.art.impact_95UL", "decouple.art.impact.sd"
  )

art.impact.table %>% View
```
...
sim.results.impact %>% 
  ...
  ungroup() %>% 
  pivot_wider(id_cols = c(sim.ix), names_from = c(scenario_name), values_from = c(Died_from_HIV, On_ART))
```
...
art.impact.deaths.data <- sim.results.impact %>% 
  filter(Year >= start_year, Year < end_year) %>% 
  #filter(Gender == 1) %>%
  group_by(scenario_name, sim.ix) %>% 
  summarize(Died_from_HIV = sum(Died_from_HIV * pop.scaling.factor)) %>% 
  pivot_wider(id_cols = c(sim.ix), names_from = c(scenario_name), values_from = c(Died_from_HIV)) %>%
  ungroup() %>% 
  mutate(baseline.art.impact = baseline.noart - baseline.art,
         decouple.art.impact = decouple.noart - decouple.art) %>% 
  mutate(art.impact = baseline.art.impact - decouple.art.impact)
...
sim.results.impact %>% 
  ...
  group_by(Gender, scenario_name, sim.ix) %>% 
  summarize(Died_from_HIV = sum(Died_from_HIV * pop.scaling.factor)) %>% 
  pivot_wider(id_cols = c(Gender, sim.ix), names_from = c(scenario_name), values_from = c(Died_from_HIV)) %>%
  ungroup() %>% 
  mutate(baseline.art.impact = baseline.noart - baseline.art,
         decouple.art.impact = decouple.noart - decouple.art) %>% 
  mutate(art.impact = baseline.art.impact - decouple.art.impact) %>% 
  ...
  ungroup() %>% 
  mutate(
    baseline.art.impact_95LL = lower_ci(baseline.art.impact, baseline.art.impact.sd/sqrt(nruns), nruns, conf_level = .95),
    baseline.art.impact_95UL = upper_ci(baseline.art.impact, baseline.art.impact.sd/sqrt(nruns), nruns, conf_level = .95),
    decouple.art.impact_95LL = lower_ci(decouple.art.impact, decouple.art.impact.sd/sqrt(nruns), nruns, conf_level = .95),
    decouple.art.impact_95UL = upper_ci(decouple.art.impact, decouple.art.impact.sd/sqrt(nruns), nruns, conf_level = .95),
    art.impact_95LL = lower_ci(art.impact, art.impact.sd/sqrt(nruns), nruns, conf_level = .95),
    art.impact_95UL = upper_ci(art.impact, art.impact.sd/sqrt(nruns), nruns, conf_level = .95)
  ) %>% 
  select(Gender, 
         baseline.art.impact, baseline.art.impact.sd, baseline.art.impact_95LL, baseline.art.impact_95UL,
         decouple.art.impact, decouple.art.impact.sd, decouple.art.impact_95LL, decouple.art.impact_95UL,
...
art.impact.inf.data <- sim.results.impact %>% 
  filter(Year >= start_year, Year < end_year) %>% 
  group_by(scenario_name, sim.ix) %>% 
  summarize(Newly.Infected = sum(Newly.Infected * pop.scaling.factor)) %>% 
  pivot_wider(id_cols = c(sim.ix), names_from = c(scenario_name), values_from = c(Newly.Infected)) %>%
  ungroup() %>% 
  mutate(baseline.art.impact = baseline.noart - baseline.art,
         decouple.art.impact = decouple.noart - decouple.art) %>% 
  mutate(art.impact = baseline.art.impact - decouple.art.impact)
  
art.impact.inf.data.agg <- art.impact.inf.data %>% 
    summarize(baseline.art.impact.sd = sd(baseline.art.impact),
            baseline.art.impact = mean(baseline.art.impact),
            decouple.art.impact.sd = sd(decouple.art.impact),
            decouple.art.impact = mean(decouple.art.impact),
...
bt_resamples <- bootstraps(art.impact.inf.data.agg, times = 500)
bt_resamples$baseline.art.impact <- map_dbl(bt_resamples$splits, bt_mean, "baseline.art.impact")
bt_resamples$decouple.art.impact <- map_dbl(bt_resamples$splits, bt_mean, "decouple.art.impact")
bt_resamples$art.impact <- map_dbl(bt_resamples$splits, bt_mean, "art.impact")
...
art.impact.inf.data.agg$baseline.art.impact_95LL <- baseline.art.impact_95[[1]]
art.impact.inf.data.agg$baseline.art.impact_95UL <- baseline.art.impact_95[[2]]
art.impact.inf.data.agg$decouple.art.impact_95LL <- decouple.art.impact_95[[1]]
art.impact.inf.data.agg$decouple.art.impact_95UL <- decouple.art.impact_95[[2]]
art.impact.inf.data.agg$art.impact_95LL <- art.impact_95[[1]]
art.impact.inf.data.agg$art.impact_95UL <- art.impact_95[[2]]

art.impact.inf.data.agg <- art.impact.inf.data.agg %>% select(
      "baseline.art.impact", "baseline.art.impact.sd", "baseline.art.impact_95LL", "baseline.art.impact_95UL",
      "decouple.art.impact", "decouple.art.impact.sd", "decouple.art.impact_95LL", "decouple.art.impact_95UL",
      "art.impact", "art.impact.sd", "art.impact_95LL", "art.impact_95UL"
    )

art.impact.inf.data.agg
```
...
sim.results.impact %>% 
  ...
  group_by(Gender, scenario_name, sim.ix) %>% 
  summarize(Newly.Infected = sum(Newly.Infected * pop.scaling.factor)) %>% 
  pivot_wider(id_cols = c(Gender, sim.ix), names_from = c(scenario_name), values_from = c(Newly.Infected)) %>%
  ungroup() %>% 
  mutate(baseline.art.impact = baseline.noart - baseline.art,
         decouple.art.impact = decouple.noart - decouple.art) %>% 
  mutate(art.impact = baseline.art.impact - decouple.art.impact) %>% 
...
```{r}
sim.results.all %>% 
  filter(Year >= 2000, scenario_name == 'baseline.art') %>% 
  group_by(Gender, sim.ix) %>% 
  summarize(Newly.Infected = sum(Newly.Infected * pop.scaling.factor)) %>% 
  group_by(Gender) %>% 
...
sim.results.all <- rbind(sim.results.decouple.art.dropout.test,
                         sim.results.decouple.art.nodropout.test,
                         sim.results.cmdinc.art.dropout.test,
                         sim.results.cmdinc.art.nodropout.test)
...
test.comparison %>% 
  ...
  summarize(Population = mean(Population), 
            ...
            Died = mean(Died),
            Died_from_HIV = mean(Died_from_HIV),
            Newly.Infected = mean(Newly.Infected)) %>% View
```


```{r}
sim.results.all %>% group_by(Year, Gender, scenario_name, sim.ix ) %>% 
