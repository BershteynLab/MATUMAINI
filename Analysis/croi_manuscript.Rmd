...
data['column.of.interest'] = data["Newly.Infected"]
  ...
  art.impact.data$art.impact.pct %>% mean(na.rm = TRUE)
  ...
  mean(bt_resamples$art.impact.ppyoa.pct)
  mean(art.impact.data.agg$art.impact.ppyoa.pct)
...
differences.table <- read_csv("differences_table_20240606.csv")
...
p.panel2.deaths.pct <- differences.table %>% 
  filter(scenario_name %in% c("baseline.art.not.cmdinc", "decouple.art.tx", "decouple.art.hivinc"),
         Ages == "15-100",
         Time < 5) %>% 
  ggplot() + 
...
p.panel2.infections.pct <- differences.table %>% 
  ...
  scale_x_continuous(breaks = c(1,2,3,4), 
                     labels = c("Pre-ART\n1985-2005", "ART Scale-up\n2005-2015", "Universal ART\n2015-2025","Future\n2025-2035")) + 
  scale_y_continuous(breaks = c(0,5,10,15,20), labels = c("0%", "5%", "10%", "15%", "20%"), expand = expansion(mult = c(0.01,.2))) + 
  #ylim(0,12) +
  labs(x = "", y = "") + 
  theme_bw(base_size=16) + 
  theme(panel.grid.major = element_blank(), 
...
p.panel3.infections.pct <- differences.table %>% 
  ...
  scale_x_continuous(breaks = c(1,2,3,4), 
                     labels = c("Pre-ART\n1985-2005", "ART Scale-up\n2005-2015", "Universal ART\n2015-2025","Future\n2025-2035")) + 
  scale_y_continuous(breaks = seq(0,20,5), labels = c("0%", "5%", "10%", "15%", "20%"), expand = expansion(mult = c(0.01,.2))) + 
  labs(x = "", y = "") + 
  theme_bw(base_size=16) + 
  theme(panel.grid.major = element_blank(), 
        panel.grid.minor = element_blank(),
...
impact.table <- read.csv("impact_of_art_table_20240606.csv")
...
### Checking on incidence over time

```{r}
sim.results.all <- rbind(sim.results.decouple.art.hivinc, sim.results.baseline.art)
