...
scenarios.list = data.frame(
  ...
  file.path = c(
    "/gpfs/scratch/citrod01/experiments/MATUMAINI/CMD_Impact/Baseline-campaign_MATUMAINI_202305_baseline_notreat-baseline___2025_03_24_11_29_25_780089",
...
metrics.disaggregated.table <- rbind(
...
scenario.names = metrics.disaggregated.table$scenario_name %>% unique 
...
for (i in metric.names){
  for (j in scenario.names) {
    ...
    h <- calculate.CI.by.metric(
      data = metrics.disaggregated.table %>% filter(Year == 2025),
      scenario.name = j,
      metric = i)
...
```{r}
data.2025.2035 <- metrics.disaggregated.table %>% 
  ...
  summarise(Newly.Infected = sum(Newly.Infected),
            ...
            Total.DALY = sum(HIV.DALY.discounted + Depression.DALY),
            CMD.treatment.cost = sum(CMD.treatment.cost),
            CMD.treatment.cost.discounted = sum(CMD.treatment.cost.discounted),
            On.art.cost = sum(On.art.cost),
            On.art.cost.discounted = sum(On.art.cost.discounted)
         ) %>% ungroup()
...
metric.names = c(
  ...
  "Total.DALY",
  "CMD.treatment.cost",
  "CMD.treatment.cost.discounted",
  "On.art.cost",
  "On.art.cost.discounted"
)
...
```{r}
hiv.averted.stats <- metrics.disaggregated.table %>% 
  filter(Year >= 2025) %>%
  group_by(scenario_name, sim.ix) %>% 
  summarize(
...
hiv.averted.stats.baseline <- metrics.disaggregated.table %>% 
...
cost.incremental <- merge(
    metrics.2025.3035 %>% 
      filter(metric == 'On.art.cost') %>% 
      mutate(On.art.cost.baseline = metrics.2025.3035 %>% filter(metric == 'On.art.cost', scenario_name == 'baseline') %>% .$metric.mean) %>% 
      mutate(On.art.cost.marginal = metric.mean - On.art.cost.baseline ) %>% 
      select(scenario_name, On.art.cost.marginal),
    metrics.2025.3035 %>% 
      filter(metric == 'CMD.treatment.cost') %>% 
      mutate(CMD.treatment.cost.baseline = metrics.2025.3035 %>% filter(metric == 'CMD.treatment.cost', scenario_name == 'baseline') %>%
               .$metric.mean) %>% 
      mutate(CMD.treatment.cost.marginal = metric.mean - CMD.treatment.cost.baseline) %>% 
      select(scenario_name, CMD.treatment.cost.marginal),
    by = c("scenario_name")
  )

cost.incremental.discounted <- merge(
    metrics.2025.3035 %>% 
      filter(metric == 'On.art.cost.discounted') %>% 
      mutate(On.art.cost.baseline = metrics.2025.3035 %>% filter(metric == 'On.art.cost.discounted', scenario_name == 'baseline') %>% .$metric.mean) %>% 
      mutate(On.art.cost.marginal = metric.mean - On.art.cost.baseline ) %>% 
      select(scenario_name, On.art.cost.marginal),
    metrics.2025.3035 %>% 
      filter(metric == 'CMD.treatment.cost.discounted') %>% 
      mutate(CMD.treatment.cost.baseline = metrics.2025.3035 %>% filter(metric == 'CMD.treatment.cost.discounted', scenario_name == 'baseline') %>%
               .$metric.mean) %>% 
      mutate(CMD.treatment.cost.marginal = metric.mean - CMD.treatment.cost.baseline) %>% 
      select(scenario_name, CMD.treatment.cost.marginal),
    by = c("scenario_name")
...
```{r}
ICER.Total.table <- HIV.Depression.Averted.Data %>% filter(metric == "Total.DALY.averted") %>% 
  merge(
    cost.incremental.discounted,
    by = c('scenario_name')
  )  %>% 
  mutate(cost.total = On.art.cost.marginal + CMD.treatment.cost.marginal) %>% 
...
### HIV-related ICERs
```{r}
ICER.HIV.table <- HIV.Depression.Averted.Data %>% filter(metric == "HIV.DALY.averted") %>% 
  merge(
    cost.incremental.discounted,
    by = c('scenario_name')
  )  %>% 
...
rbind(ICER.HIV.table, ICER.Total.table) %>% write_csv("Impact_Manuscript_Figures/ICER_Data.csv")
``` 
...
```{r}
# ICERs are here
ICER.table <- read_csv("Impact_Manuscript_Figures/ICER_Data.csv")
# This table has the DALYs averted
HIV.Depression.Averted.Data <- read_csv("Impact_Manuscript_Figures/HIV_Depression_Averted_Data.csv")
# This table has the costs
metrics.aggregated <- read_csv("Impact_Manuscript_Figures/HIV_Depression_Data.csv")
...
impact.DALYs.averted <- rbind(
  HIV.Depression.Averted.Data %>% filter(metric %in% c("HIV.DALY.averted", "Depression.DALY.averted", "Total.DALY.averted")),
  metrics.aggregated %>% filter(metric %in% c("On.art.cost.discounted", "CMD.treatment.cost.discounted"))
  )
...
impact.pivot <- impact.pivot %>% 
  mutate(
    On.art.cost.baseline = metrics.aggregated %>% 
      filter(metric == "On.art.cost.discounted", scenario_name == 'baseline') %>% 
      .$metric.mean    
  ) %>% 
  mutate(
    On.art.cost.marginal = metric.mean_On.art.cost.discounted - On.art.cost.baseline
  ) %>% 
  mutate(
    metric.mean.total.cost = metric.mean_CMD.treatment.cost.discounted + On.art.cost.marginal,
    metric.lb.total.cost = metric.lb_CMD.treatment.cost.discounted + On.art.cost.marginal,
    metric.ub.total.cost = metric.ub_CMD.treatment.cost.discounted + On.art.cost.marginal
  )
```
...
plot.Total.ICER <-  ggplot() + 
  geom_errorbar(
    ...
    mapping = aes(x = (metric.mean.total.cost)/1e6,
                  ymin = metric.lb_Total.DALY.averted, 
                  ymax = metric.ub_Total.DALY.averted,
                  y = metric.mean_Total.DALY.averted,
