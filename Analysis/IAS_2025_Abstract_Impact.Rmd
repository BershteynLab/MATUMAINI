...
### HIV deaths, HIV infections, Depression episodes averted
...
daly %>% 
  filter(year >= 2025) %>% 
  group_by(sim.ix) %>% 
  summarize(HIV.DALYs = sum(daly_future_discounted)) %>% 
  .$HIV.DALYs %>% mean
```
...
```
```{r}
ggsave("IAS_impact_plot.pdf", plot.averted, width = 8, height = 4, units = 'in')
```
