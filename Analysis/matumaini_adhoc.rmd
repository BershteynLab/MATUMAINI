...
# 2. Trigger depression screening/treatment upon HIV diagnosis
res.path.cmd2 = "/gpfs/scratch/citrod01/experiments/MATUMAINI/Baseline-campaign_MATUMAINI_202305-CMD_screen___2023_07_27_13_00_04_855322"
# 3. Trigger depression recovery upon VLS - 
res.path.cmd2 = "/gpfs/scratch/citrod01/experiments/MATUMAINI/Baseline-campaign_MATUMAINI_202305-CMD_vls___2023_07_28_19_37_58_913010"

# 4. Updated mathematical model of CMD
res.path.cmd2 = "/gpfs/scratch/citrod01/experiments/MATUMAINI/Baseline-campaign_MATUMAINI_202305-math___2023_09_01_17_59_02_621173"
...
p
```
How many people in each of the categories?
...
dat.cmd <- sim.results.cmd2 %>% filter(Age > 18) %>% 
  group_by(Year, Gender, sim.id, scenario_name, IP_Key.CMDStatus) %>% 
  summarize(Population = sum(Population), .groups = "keep") %>% 
  ungroup()

dat.cmd.mean <- dat.cmd %>% 
  group_by(Year, Gender, IP_Key.CMDStatus) %>% 
  summarize(Population = mean(Population))

# Plot  
p = ggplot() +
  geom_line(data = dat.cmd.mean, 
               size=2.0, aes(x=Year, y=Population, color = IP_Key.CMDStatus)) +
  #geom_errorbar(data= data.mean, mapping = aes(x = Year, ymin = lb, ymax = ub), color="black", width=2, size=1) + 
    facet_wrap(~ Gender, ncol=2) +
    xlab("Year")+ylab("") + 
    theme_bw(base_size=16) +
...
```{r}
cmd.data.age.mean %>% filter(Year == 2004) %>% arrange(Gender, AgeBin_index) %>% head(14)
```
```{r}
cmd.data.age <- sim.results.cmd2 %>% 
  pivot_wider(id_cols = c(Year, Age, Gender, sim.id, scenario_name),
...
# Label each age by its bin
cmd.data.age.hivpos <- cmd.data.age %>% mutate(
  AgeBin = cut(Age, breaks = age_bins, right = FALSE)
  ) %>%
  filter(!is.na(AgeBin))
cmd.data.age.hivpos$AgeBin_index = factor(cmd.data.age.hivpos$AgeBin,labels = 1:length(age_labels))

cmd.data.age.hivpos.mean <- cmd.data.age.hivpos %>% group_by(Year, AgeBin_index, AgeBin, Gender, scenario_name) %>% 
  summarize(mean.CMD_prev = mean(CMD_prev),
              sd.CMD_prev = sd(CMD_prev),
              .groups = 'keep') %>% 
  mutate(lower = max(mean.CMD_prev - 2* sd.CMD_prev, 0), 
...
cmd.data.age.hivpos.mean %>% filter(Year == 2004) %>% arrange(Gender, AgeBin_index) %>% head(14)
```
```{r}
cmd.data.age <- sim.results.cmd2 %>%
  mutate(Susceptible = Population - Infected) %>% 
  pivot_wider(id_cols = c(Year, Age, Gender, sim.id, scenario_name),
              names_from = c(IP_Key.CMDStatus),
              values_from = c(Susceptible)) %>% 
  group_by(Year, Age, Gender, sim.id, scenario_name) %>% 
  summarize(CMD_pos = sum(CMD_pos), 
            Population = sum(CMD_pos + CMD_neg + CMD_recovered + CMD_treated), 
            .groups = 'keep') %>% 
...
# Label each age by its bin
cmd.data.age.hivneg <- cmd.data.age %>% mutate(
  AgeBin = cut(Age, breaks = age_bins, right = FALSE)
  ) %>%
  filter(!is.na(AgeBin))
cmd.data.age.hivneg$AgeBin_index = factor(cmd.data.age.hivneg$AgeBin,labels = 1:length(age_labels))

cmd.data.age.hivneg.mean <- cmd.data.age.hivneg %>% group_by(Year, AgeBin_index, AgeBin, Gender, scenario_name) %>% 
  summarize(mean.CMD_prev = mean(CMD_prev),
              sd.CMD_prev = sd(CMD_prev),
              .groups = 'keep') %>% 
  mutate(lower = max(mean.CMD_prev - 2* sd.CMD_prev, 0), 
...
cmd.data.age.hivneg.mean %>% filter(Year == 2004) %>% arrange(Gender, AgeBin_index) %>% head(14)
```
...
```{r}
# 2. Trigger depression screening/treatment upon HIV diagnosis
event.rec.path = "/gpfs/scratch/citrod01/experiments/MATUMAINI/Baseline-campaign_MATUMAINI_202305-CMD_screen___2023_07_27_13_00_04_855322/Simulation_0G7NTEZE/output/ReportEventRecorder.csv"

event.rec.cmd = read_csv(event.rec.path)

event.rec.cmd$Event_Name %>% unique()
