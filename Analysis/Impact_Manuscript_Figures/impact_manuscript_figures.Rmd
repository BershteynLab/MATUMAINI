...
sim.results <- EMODAnalyzeR::read.simulation.results.  experiment_path = experiment.path,
  scenario_name = scenario.name,
...
rbind(ICER.discounted.total.table, ICER.HIV.table) %>% write_csv("/gpfs/data/bershteynlab/EMOD/citrod01/MATUMAINI/Analysis/Impact_Manuscript_Figures/ICER_Data_20250329.csv")
``` 
...
HIV.Depression.Averted.Data <- read_csv("HIV_Depression_Averted_Data_20250329.csv")
```
...
HIV.Depression.Averted.Data.temp <- HIV.Depression.Averted.Data %>% 
  ...
  filter(metric %in% c("Depression.episodes.averted.pct",
    ...
    mutate(scenario_ix = case_match(scenario_name, 
           ...
           metric = factor(metric, levels = c("Depression.episodes.averted.pct",
                                            "Newly.Infected.averted.pct",
                                            "Died_from_HIV.averted.pct"))
         )
...
```{r}
ggsave("Figure_2_Treatment_Plot_20250331.pdf", plot.treatment, width = 3, height = 4, units = 'in')
```
...
plot.ceac.total <- ceac.table.discounted %>% 
  ...
  scale_x_continuous(name = "Incremental Cost-Effectiveness Ratio\n($US/DALY)",
                     expand = expansion(mult = c(0.02,.05))) + 
