...
h <- sim.cmd %>% filter(sim.id == cmd.data$sim.id %>% first) %>% 
  pivot_wider(id_cols = c(Year, Age, Gender, sim.id, scenario_name),
              names_from = c(IP_Key.CMDStatus),
              values_from = c(Population)) %>% 
  mutate(CMD_pop = CMD_neg + CMD_pos + CMD_remission + CMD_remission_treated) %>% 
  select(Year, Age, Gender, CMD_pop, CMD_pos)

h

h %>% group_by(Year, Gender) %>% 
  summarize(CMD_pop = sum(CMD_pop), CMD_pos = sum(CMD_pos))

```

```{r}
h %>% group_by(Year, Age, Gender) %>% 
  summarize(CMD_pop = sum(CMD_pop), CMD_pos = sum(CMD_pos)) %>% 
  mutate(CMD_prev = CMD_pos/CMD_pop)
...
p <- cmd.data.age %>% emodplot.by_age_gender("CMD_pos", "Population", 
                       subset_years, 
                       age_bins, 
                       yaxis_lab = "CMD Prevalence", 
                       title = "CMD Prevalence by Age")
