...
#res_path = "/gpfs/scratch/citrod01/experiments/MATUMAINI/Baseline-campaign_Nyanza_baseline_202301-MH_test___2023_05_10_10_37_33_099470"
res_path = "/gpfs/data/bershteynlab/EMOD/citrod01/MATUMAINI/202301_Nyanza_Baseline_MalariaOngoing/MH_test/Baseline-campaign_Nyanza_baseline_202301-MH_test/ReportHIVByAgeAndGender"
sim.cmd <- EMODAnalyzeR::read.simulation.results(
  results_path = res_path,
  scenario_name = 'baseline',
  summarize_columns = c("Newly.Infected", "Newly.Tested.Positive",
...
p + 
  geom_point(data = obs.cmdprev.sheet, 
             mapping = aes(x = AgeBin_index, y = CMDPrevalence))
```
...
data <- cmd.data.age %>% mutate(
  AgeBin = cut(Age, breaks = age_bins, right = FALSE)
  ) %>%
  filter(!is.na(AgeBin))
data$AgeBin_index = factor(data$AgeBin,labels = 1:(length(age_bins)-1))
# Pick out the numerator and denominator
data['Numerator'] = data["CMD_pos"]
...
ggplot() + 
  # plot means
  geom_point(
    data = data.mean %>% filter(Year == 2004),
    mapping = aes(x = AgeBin_index, y = mean.Prevalence, color = scenario_name),
    size=1
  ) + facet_grid(cols = vars(Gender), rows = vars(Year)) + 
  geom_point(data = obs.cmdprev.sheet, 
